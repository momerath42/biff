...
(defn passed-recaptcha?
  "Checks if the given request contains a valid Recaptcha token.

  (:g-recaptcha-response params) should contain the token (set by the Recaptcha
  client library automatically. If (secret :recaptcha/secret-key) is nil,
  bypasses the Recaptcha test and returns true.

  For Recaptcha v2 tests, the response will be pass or fail. For v3 tests,
  Google returns a score between 0 and 1. In this case, the function returns
  true if (<= threshold score)."
  [{:keys [biff/secret biff.recaptcha/threshold params]
    :or {threshold 0.5}
    :as req}]
  (auth/passed-recaptcha? req))

(def recaptcha-disclosure
  "A Rum data structure that contains a disclosure statement, which should be
